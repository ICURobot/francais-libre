...
async function cleanupBoulangerieDuplicates() {
  console.log('🧹 Cleaning up duplicate "Je vais à la boulangerie." entries...\n')
    ...
    if (response.ok) {
      ...
      if (data && data.length > 0) {
        ...
        if (myleneEntries.length > 0 && andreEntries.length > 0) {
          ...
          console.log(`✅ Cleanup complete for: "Je vais à la boulangerie."`)
          
        } else if (myleneEntries.length > 0 && andreEntries.length === 0) {
          console.log(`⚠️ Only Mylène entries found for: "Je vais à la boulangerie."`)
          console.log('   This means our André generation failed or was overwritten')
        } else if (myleneEntries.length === 0 && andreEntries.length > 0) {
          console.log(`✅ Only André entries found for: "Je vais à la boulangerie."`)
