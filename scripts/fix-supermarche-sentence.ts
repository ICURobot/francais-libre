...
const backupDir = path.join(process.cwd(), 'audio-backup', 'supermarche-sentence-fix')
...
// Main function to fix "Je fais mes courses au supermarché." audio
async function fixSupermarcheSentence() {
  console.log('🎵 Fixing missing audio for "Je fais mes courses au supermarché."...\n')
  console.log('✅ Main word "le supermarché" will be left alone (already working)\n')
  ...
  try {
    console.log(`--- Processing: "${supermarcheSentence.text}" ---`)
    
    // Generate audio
    const audioResult = await generateAudioWithAndre(supermarcheSentence.text)
    ...
    const uploadSuccess = await uploadToSupabase(
      supermarcheSentence.text,
      audioResult.audioData,
      audioResult.fileName,
      supermarcheSentence.category,
      supermarcheSentence.lessonId
    )

    if (uploadSuccess) {
      console.log(`✅ SUCCESS: "${supermarcheSentence.text}" now has audio!`)
      console.log(`🎯 Test the vocabulary card - example sentence should now play audio`)
      console.log(`✅ Main word "le supermarché" was left alone (already working)`)
    } else {
      console.log(`❌ FAILED: "${supermarcheSentence.text}"`)
    }

  } catch (error) {
    console.error(`❌ ERROR processing "${supermarcheSentence.text}":`, error)
  }

  console.log(`\n💾 Local backup saved in: ${backupDir}`)
}

fixSupermarcheSentence()
