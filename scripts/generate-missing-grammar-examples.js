...
// Only the missing grammar examples
...
// Main function to generate only the missing grammar examples
async function generateMissingGrammarExamples() {
  console.log('🎵 Generating only the missing grammar examples for Lesson 10...\n')
  ...
  console.log(`📚 Total items to process: ${missingExamples.length}`)
  ...
  for (let i = 0; i < missingExamples.length; i++) {
    const item = missingExamples[i]
    console.log(`\n🔄 Processing ${i + 1}/${missingExamples.length}: "${item.text}"`)
  ...
  console.log(`📁 Total processed: ${missingExamples.length}`)
  
  if (errorCount > 0) {
    console.log('\n❌ Failed items:')
    results.forEach((result, index) => {
      if (!result.success) {
        console.log(`  - ${missingExamples[index].text}: ${result.error}`)
      }
    })
  }
  
  console.log('\n🎉 Missing grammar examples audio generation complete!')
...
if (require.main === module) {
  generateMissingGrammarExamples().catch(console.error)
}

module.exports = { generateMissingGrammarExamples }
